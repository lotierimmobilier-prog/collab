...
export async function POST(req: NextRequest) {
  ...
  const inner = (typeof body?.html === "string" && body.html.trim())
    ? body.html
    : `<h2 style="color:#B8966A;margin-top:0">Test de configuration réussi !</h2>
        <p>Votre serveur SMTP est correctement configuré pour <strong>Lotier Immobilier</strong>.</p>
        <p style="color:#6b7280;font-size:12px">Destinataire : ${to}<br/>Envoyé depuis : ${cfg.from}</p>`;
  const html = renderBrandedEmail({ subject, contentHtml: inner });
