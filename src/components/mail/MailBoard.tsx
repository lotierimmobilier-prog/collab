...
function ComposeModal({ accounts, gmailConfigs, labels, onClose, onSend, replyTo, initialBody, initialCc, initialAttachments, defaultFromEmail }: {
  ...
  return (
    <>
      {/* z-index au-dessus du popup de lecture (zIndex 110) pour que la fenêtre
          de rédaction (réponse / transfert) passe DEVANT le mail ouvert. */}
      <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 120 }} />
      <div style={{ position: "fixed", ...frame, background: "#fff", borderRadius: size === "full" ? 12 : 14, zIndex: 121, boxShadow: "0 20px 60px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        {/* Header */}
